...
bool ImageProcessor::initialize() {
  ...
  detector_ptr = FastFeatureDetector::create("FAST");
...
void ImageProcessor::addNewFeatures() {
  ...
  for (const auto& features : *curr_features_ptr) {
    for (const auto& feature : features.second) {
      ...
      for(int i = up_lim; i <bottom_lim;i++ )
...
void ImageProcessor::integrateImuData(
  ...
  cv::Matx31d mean_ang_vel(0.0, 0.0, 0.0);
  for (auto iter = begin_iter; iter < end_iter; ++iter)
    mean_ang_vel +=  cv::Matx31f(iter->angular_velocity.x,
  ...
  cv::Matx31d cam0_mean_ang_vel = R_cam0_imu.t() * mean_ang_vel;
  cv::Matx31d cam1_mean_ang_vel = R_cam1_imu.t() * mean_ang_vel;
